// DOS FRAMEWORK: Modal Theme Alignment & Snake Game Fix - Browser Console Injection
console.log('🎯 DOS FRAMEWORK: Injecting Modal Theme Alignment & Snake Game Fix...');
...
// Snake Game Logic
let dosSnakeGame = {
    canvas: null, ctx: null, snake: [{x: 200, y: 200}], food: {x: 0, y: 0},
    dx: 0, dy: 0, score: 0, gameRunning: false, gameLoop: null, gridSize: 20, tileCount: 20, speed: 150
};
...
function initializeDOSSnakeGame() {
    ...
    dosSnakeGame.canvas = canvas; dosSnakeGame.ctx = ctx;
    dosSnakeGame.snake = [{x: 200, y: 200}]; dosSnakeGame.dx = 0; dosSnakeGame.dy = 0; dosSnakeGame.score = 0; dosSnakeGame.gameRunning = false;
    generateDOSFood(); drawDOSSnakeGame();
...
function pauseDOSSnakeGame() {
    if (!dosSnakeGame.gameRunning) return;
    dosSnakeGame.gameRunning = false; clearInterval(dosSnakeGame.gameLoop);
    const startBtn = document.getElementById('dos-start-btn');
    if (startBtn) { startBtn.textContent = 'RESUME'; startBtn.disabled = false; startBtn.style.opacity = '1'; }
}

function resetDOSSnakeGame() {
    dosSnakeGame.gameRunning = false;
    if (dosSnakeGame.gameLoop) { clearInterval(dosSnakeGame.gameLoop); dosSnakeGame.gameLoop = null; }
    dosSnakeGame.snake = [{x: 200, y: 200}]; dosSnakeGame.dx = 0; dosSnakeGame.dy = 0; dosSnakeGame.score = 0;
    generateDOSFood(); drawDOSSnakeGame();
    const scoreElement = document.getElementById('dos-snake-score');
    if (scoreElement) scoreElement.textContent = dosSnakeGame.score;
    const startBtn = document.getElementById('dos-start-btn');
    if (startBtn) { startBtn.textContent = 'START'; startBtn.disabled = false; startBtn.style.opacity = '1'; }
}

function updateDOSSnakeGame() {
    if (!dosSnakeGame.gameRunning) return;
    const head = { x: dosSnakeGame.snake[0].x + dosSnakeGame.dx, y: dosSnakeGame.snake[0].y + dosSnakeGame.dy };
    if (head.x < 0 || head.x >= 400 || head.y < 0 || head.y >= 400) { dosGameOver(); return; }
    for (let segment of dosSnakeGame.snake) {
        if (head.x === segment.x && head.y === segment.y) { dosGameOver(); return; }
    }
    ...
    if (head.x === dosSnakeGame.food.x && head.y === dosSnakeGame.food.y) {
        ...
        if (dosSnakeGame.score % 50 === 0 && dosSnakeGame.speed > 80) {
            dosSnakeGame.speed -= 5; clearInterval(dosSnakeGame.gameLoop);
            dosSnakeGame.gameLoop = setInterval(updateDOSSnakeGame, dosSnakeGame.speed);
        }
    } else { dosSnakeGame.snake.pop(); }
    drawDOSSnakeGame();
}

function drawDOSSnakeGame() {
    const ctx = dosSnakeGame.ctx; const isDarkMode = document.body.classList.contains('dark');
    ctx.fillStyle = isDarkMode ? 'hsl(0 0% 3.9%)' : 'hsl(0 0% 100%)'; ctx.fillRect(0, 0, 400, 400);
    ctx.strokeStyle = isDarkMode ? 'hsl(217.2 32.6% 17.5%)' : 'hsl(210 40% 96%)'; ctx.lineWidth = 1;
    for (let i = 0; i <= 20; i++) {
    ...
    dosSnakeGame.snake.forEach((segment, index) => {
        ...
        ctx.lineWidth = 1; ctx.strokeRect(segment.x + 1, segment.y + 1, 18, 18);
    });
    const time = Date.now() * 0.005; const pulse = Math.sin(time) * 0.1 + 0.9;
    const foodSize = 18 * pulse; const offset = (18 - foodSize) / 2;
    ...
    ctx.shadowColor = isDarkMode ? 'hsl(0 84% 60%)' : 'hsl(0 84% 60%)'; ctx.shadowBlur = 8;
    ctx.fillRect(dosSnakeGame.food.x + 1 + offset, dosSnakeGame.food.y + 1 + offset, foodSize, foodSize);
    ctx.shadowBlur = 0;
}
...
function handleDOSSnakeKeydown(event) {
    ...
    switch (key) {
        case 'arrowup': case 'w': if (dosSnakeGame.dy === 0) { dosSnakeGame.dx = 0; dosSnakeGame.dy = -20; } break;
        case 'arrowdown': case 's': if (dosSnakeGame.dy === 0) { dosSnakeGame.dx = 0; dosSnakeGame.dy = 20; } break;
        case 'arrowleft': case 'a': if (dosSnakeGame.dx === 0) { dosSnakeGame.dx = -20; dosSnakeGame.dy = 0; } break;
        case 'arrowright': case 'd': if (dosSnakeGame.dx === 0) { dosSnakeGame.dx = 20; dosSnakeGame.dy = 0; } break;
        case ' ': event.preventDefault(); pauseDOSSnakeGame(); break;
    }
}
...
function dosGameOver() {
    ...
    const ctx = dosSnakeGame.ctx; const isDarkMode = document.body.classList.contains('dark');
    ctx.fillStyle = isDarkMode ? 'rgba(0, 0, 0, 0.8)' : 'rgba(255, 255, 255, 0.9)'; ctx.fillRect(0, 0, 400, 400);
    ctx.fillStyle = isDarkMode ? 'hsl(0 0% 98%)' : 'hsl(222.2 84% 4.9%)';
    ctx.font = 'bold 24px "SF Mono", "Monaco", "Inconsolata", "Roboto Mono", monospace';
    ctx.textAlign = 'center'; ctx.fillText('GAME OVER', 200, 180);
...
window.launchSnakeGame = function() { createDOSCompliantSnakeGameModal(); };
...
console.log('🚀 DOS FRAMEWORK: Modal Theme Alignment & Snake Game Fix ACTIVE');
console.log('📋 FEATURES: ✅ Theme-aware modals ✅ Functional Snake Game ✅ DOS aesthetics');
